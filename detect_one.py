...
def show_results(img, xywh, conf, landmarks, class_num):
    ...
    for i in range(4):
        x[i] = int(landmarks[2 * i] * w);
        y[i] = int(landmarks[2 * i + 1] * h);

    cv2.line(img, (x[0], y[0]), (x[2], y[2]), (255, 0, 0), tl + 1)
    cv2.line(img, (x[1], y[1]), (x[3], y[3]), (0, 255, 0), tl + 1)

    ll = math.sqrt((x[0] - x[2]) ** 2 + (y[0] - y[2]) ** 2)
    ww = math.sqrt((x[1] - x[3]) ** 2 + (y[1] - y[3]) ** 2)
    tf = max(tl - 1, 1)  # font thickness
    label = 'airplane: ' + str(conf)[:5] + ' l/w: '+str(ll/ww)[:5]
